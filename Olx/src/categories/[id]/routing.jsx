...
export default function Routing() {
        ...
        {[...Array(6)].map((_, index) => (
          <div key={index} className="bg-white shadow-xl rounded-lg overflow-hidden">
            ...
            <div className="p-6">
              ...
              <Skeleton height={25} width={200} className="mt-3" />
  ...
  <div className="w-full max-w-7xl grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-8">
    {ads.map(ad => (
      <div key={ad.id} className="bg-slate-700 shadow-xl rounded-lg overflow-hidden transform ease-in-out">
        {Array.isArray(ad.photo) && ad.photo.length > 1 ? (
          ...
          ad.photo && (
            <img src={Array.isArray(ad.photo) ? ad.photo[0] : ad.photo} alt="Post Image" className="w-full h-48 object-cover rounded-t-lg transition-transform" />
          )
        )}
        ...
        <div className="p-6">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-semibold text-white">{ad.createdAt?.toDate().toLocaleDateString()}</span>
          </div>

          <h2 className="mt-3 text-2xl font-semibold text-white">{ad.adTitle}</h2>
          <p className="text-sm text-white mt-2">{ad.adDescription}</p>

          <p className="text-lg text-white font-semibold mt-3">Price: <span className="text-green-600">{ad.adPrice}</span></p>
          ...
          <div className="flex items-center mt-4">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-white">{ad.userData ? ad.userData.username : 'Anonymous'}</p>
            </div>
          </div>
        </div>
      </div>
